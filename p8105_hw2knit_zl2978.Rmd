...
```
There are `r filter(NYC_reformat_transit, route_name == "A") %>% distinct(line, station_name) %>% nrow()` distinct stations serve the A train, and `r filter(NYC_reformat_transit, route_name == "A") %>% filter(ada == T) %>% distinct(line, station_name) %>%  nrow()` are ADA compliant.
...
The last one is the dataset that merge all three datasets and information together. It contains the information about the year, month, president type, the number of republican and democratic governors, the number of republican and democratic senators and the number of republican and democratic representatives on a given date, closing values of the stock index, and the unemployment rate. Key variables are president, unempolyment_rate and close. There are `r nrow(merge_df)` rows and `r ncol(merge_df)` columns. The range of the year is from `r min(pull(merge_df,year))` to `r max(pull(merge_df,year))`.
